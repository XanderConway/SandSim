...
public class FireCell : Cell
{
    ...
    public FireCell(Color col, float spread_chance, int life_time)
    {
    ...
    public override void move(short x, short y)
    {
        ...
        if (time_to_die())
        {
            ...
            //    Grid.swap(x, y, x, y - 1);
            //}
            Grid.grid[x, y] = new SmokeCell(new Color(0.6f, 0.6f, 0.6f), 2, new Vector2(0, -0.1f));
            return;
        }
    ...
    private bool set_on_fire(int x, int y)
    {
        ...
        if (this.spread_chance > burn_function())
        {
            if (Grid.in_bound(x, y) && Grid.grid[x, y].flammability > 0)
            {
                ...
                fire_cell.glow_timer += this.glow_timer + 8;
